...
## What this is
A single-page interactive web app mapping every DPIIT-recognised startup in Madhya Pradesh (India) on a Leaflet map (~8,000, refreshed weekly from the Startup India portal), with a searchable/filterable list and company detail cards.
...
3. **Filters** — district, industry and sector dropdowns, "has contacts only" toggle. The list renders 200 cards at a time (Show more).
